...
{
    if (Pstream::master())//mao update and sovle at the parallel compute master process
    {
    ...
    Pstream::scatter(motionState_);//mao store the motion state for next time step use,Scatter data. Distribute without modification. Reverse of gather.
}
