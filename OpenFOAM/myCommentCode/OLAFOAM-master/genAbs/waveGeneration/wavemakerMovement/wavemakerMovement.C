...
#if OFFLAVOUR == 1  //for different OF version
...
void wavemakerMovement::updateCoeffs()
{
    ...
    const scalarField alphaCell = alpha.boundaryField()[patchId]; // Alpha of the patch face 0 1
    ...
    if ( tSmooth_ > 0.0 && currTime < tSmooth_ )//only effect between the zero and tSmooth
    {
    ...
    {
        if ( indexF == 0) //initial condication
        {
    ...
    if ( wavemakerType_ == "Piston" )
    {
        forAll( auxPoints, pInd )
        {
            ...
            auxPoints[pInd] = pDisp * meanAngle_; //update the auxPoints
        }
    }
    else if ( wavemakerType_ == "Flap" )
    {
