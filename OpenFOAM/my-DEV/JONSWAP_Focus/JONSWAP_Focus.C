...
defineTypeNameAndDebug(JONSWAP_Focus, 0);
addToRunTimeSelectionTable(waveSpectra, JONSWAP_Focus, waveSpectra);

// * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * //

scalarField JONSWAP_Focus::spectralValue
(
    const scalar& Hs,
    const scalar& Tp,
    const scalar& gamma,
...
void JONSWAP_Focus::set(Ostream& os)
{
    ...
    scalar A_Focus(readScalar(dict_.lookup("A_Focus")));
    ...
    //calculate the sum of the waveSpectra
    scalar Sum_Spectra = 0;
    for (int i = 1; i < N + 1; i++)
    {
        Sum_Spectra += S[i-1]*(nodeFrequency[i] - nodeFrequency[i - 1]);
    }
    ...
    for (int i = 1; i < N + 1; i++)
    {
        ...
        amp_[i - 1] = A_Focus*(0.5*(S[i-1] + S[i])* (nodeFrequency[i] - nodeFrequency[i - 1]))/Sum_Spectra;
