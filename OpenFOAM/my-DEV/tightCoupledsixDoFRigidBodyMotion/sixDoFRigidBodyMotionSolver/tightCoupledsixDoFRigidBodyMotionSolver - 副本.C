...
//mao points0 is the point in constant/polyMesh
    ...
    else
    {
        ...
        if(icorr==0)
            ...
            f.calcForcesMoment();
             ...
             fPrevIter=f.forceEff();
             mPrevIter=f.momentEff();
            motion_.update
           (
            firstIter,
           ...
           *(
           ...
           Info << "loop times1="<<&icorr<<nl<<endl;
        	}
        else if (icorr==1)
        	{
            ...
            rfi=(f.forceEff()-fPrevIter)/(motion_.mass()*mag(g.value()));
           ...
           *(
            ...
            mPrevIter=f.momentEff();
            rfiPre=rfi;
            Info << "loop times2="<<icorr<<nl<<endl;
          }
        else if (icorr>=2)
          {
            ...
            frelax=frelax1*((rfiPre&(rfi-rfiPre))/(magSqr(rfi-rfiPre)));
            frelax1=frelax;
            rfiPre=rfi;
            vector Frelax=frelax*f.forceEff()+(1-frelax)*fPrevIter;
            vector Mrelax=frelax*f.momentEff()+(1-frelax)*mPrevIter;	
          motion_.update
          (
            firstIter,
            ramp*(Frelax+ motion_.mass()*g.value()),
            ramp
           *(
               Mrelax
             + motion_.mass()*(motion_.momentArm() ^ g.value())
           ...
           fPrevIter=f.forceEff();
           mPrevIter=f.momentEff();
           Info << "loop times3="<<icorr<<nl<<endl;
          }
//        first = false;
//        motion_.update
//        (
...
//            t.deltaT0Value()
//        );
//        vector fPrevIter=f.forceEff();
//        vector mPrevIter=f.momentEff();
//
//        forces f("forces", db(), forcesDict);
//
//        f.calcForcesMoment();
...
//        vector rfiPre=rfi;
//        vector rfi=(f.forceEff()-fPrevIter)/(motion_.mass()*g.value());
//        scalar frelax0=frelax;
//        scalar frelax=frelax0*((rfiPre&(rfi-rfiPre)/magSqr(rfi-rfiPre));
//        if (icorr>=3)
//        {
//
//        vector Frelax()=frelax*f.forceEff()+(1-frelax)*fPrevIter
