...
{
    defineTypeNameAndDebug(PTO_Angle, 0);
...
Foam::sixDoFRigidBodyMotionRestraints::PTO_Angle::
PTO_Angle
(
    const word& name,
...
void
Foam::sixDoFRigidBodyMotionRestraints::PTO_Angle::restrain
(
...
//-------------------------
    interpolationTable<scalar> dataTable = interpolationTable<scalar>("Data_Angle.txt");



    // Damping of along axis angular velocity only
    restraintMoment = dataTable(time)*a;
...
bool Foam::sixDoFRigidBodyMotionRestraints::PTO_Angle::read
(
    const dictionary& sDoFRBMRDict
)
{
...
void Foam::sixDoFRigidBodyMotionRestraints::PTO_Angle::write
(
    Ostream& os
) const
{
