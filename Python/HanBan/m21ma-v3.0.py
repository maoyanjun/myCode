...
print(dir_tmp)
dir_wind = r'E:\Mooring\Hanban\Hanbanxibo\TVDMA\MA\wind'
Wind_Ref = os.listdir(dir_wind)
dir_RefMA = r'E:\Mooring\Hanban\Hanbanxibo\TVDMA\MA\RefMASetups'
...
for i in range(len(dir_tmp)):
    ...
    for k in range(len(Wind_Ref)):
        orgLine1 = '         file_name = |.\H0.8Tp12m0.9thet180.dfs2|\n'
        changeLine1 = '         file_name = |.\\'+dir_MAcase_tmp +'.dfs2|\n'
        orgLine2 = '         file_name = |..\..\wind30-3.dfs0|\n'
        changeLine2 = '         file_name = |E:\\Mooring\\Hanban\\Hanbanxibo\\TVDMA\\MA\\wind\\'+Wind_Ref[k]+'|\n'
        fileName = dir_RefMA+'/30speed3.m21ma'
        copyDirname  = dir_MAcase_tmp+'/'+dir_MAcase_tmp+Wind_Ref[k][:-5]+'.m21ma'
        MA_casefile = dir_MAcase_tmp+Wind_Ref[k][:-5]+'.m21ma'
        def_functions2.changetext2(orgLine1,changeLine1,orgLine2,changeLine2,fileName,copyDirname)

        #shutil.copy(dir_wind+'/'+Wind_Ref[i],dir_MAcase_tmp+'/'+MA_casefile)
    ...
    #            pyautogui.moveTo(45,121,duration=0.2)
                pyautogui.click(52,81,button='left')
                time.sleep(1)
                pyautogui.click(120,344,button='left')
    ...
    #            pyautogui.hotkey('altleft','f4')
                ...
                data_max = Result_tmp.iloc[int(startTime):,2:].max()
                ...
                MotionMax =np.zeros(6)
                MotionSigAbs =np.zeros(6)
                if file_Result_dfs[k][:-5] == 'Vessel_1_motions':
                    dataM_np = Result_tmp.values
                    for l in range(6):
                        MotionSig[l],MotionMax[l] = def_functions2.upCross1(dataM_np[int(startTime):,l+2])
                        MotionSigAbs[l] = def_functions2.upCross2(dataM_np[int(startTime):,l+2])
                    ...
                    MotionMaxStr = str(MotionMax)
                    MotionMaxStr = MotionMaxStr.strip(']')
                    MotionMaxStr = MotionMaxStr.strip('[')
                    MotionMaxStr = ','.join(MotionMaxStr.split())
                    with open('..\\..\\'+ file_Result_dfs[k][:-4]+'_maxH_sig.csv','a') as file_R:
                        file_R.write(Results_dir[2:5]+','+Results_dir[7:9]+','+Results_dir[13:16]+','+Results_dir[20:23]+','+Results_dir[24]+','+ MotionMaxStr+','+MotionSigStr+','+MotionSigAbsStr+'\n')
                    file_R.close()

                #追加到汇总报告,给的绝对路径和文件名,Vessel_motion.csv,fender_max_force.csv,
                # bollard_max_force.csv,line_max_force.csv
                boolBreak=0
                if file_Result_dfs[k][:-5] == 'lforces':
                    if Result_tmp.shape[0] < 20000:
                        boolBreak = 1
                    with open('..\\..\\'+ file_Result_dfs[k][:-4]+'_max.csv','a') as file_R:
                        file_R.write(Results_dir[2:5]+','+Results_dir[7:9]+','+Results_dir[13:16]+','+Results_dir[20:23]+','+Results_dir[24]+','+str(boolBreak)+','+ data_str+'\n')
                    file_R.close()
                else:
                    with open('..\\..\\'+ file_Result_dfs[k][:-4]+'_max.csv','a') as file_R:
                        file_R.write(Results_dir[2:5]+','+Results_dir[7:9]+','+Results_dir[13:16]+','+Results_dir[20:23]+','+Results_dir[24]+','+str(boolBreak)+','+ data_str+'\n')
                ...
                print('%s is not found, continue to next file',file_Result_dfs[k][:-5])
                continue
